...
class ReportMgr(ReportMgrBase):
    ...
    def _report_step(self, lr, step, train_stats=None, valid_stats=None):
        ...
        if valid_stats is not None:
            self.log('Valid perp: %g' % valid_stats.ppl())
            logger.info(
                ('word_acc: %4.2f parlabel_acc: %4.2f chlabel_acc: %4.2f par_acc: %4.2f ch_xent: %4.2f')
                % (valid_stats.word_accuracy(), valid_stats.label_accuracy(), valid_stats.ch_label_accuracy(), valid_stats.par_accuracy(), valid_stats.ch_order_xent()))
            logger.info('BLEU: {}'.format(valid_stats.bleu))
