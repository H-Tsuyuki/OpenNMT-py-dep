...
class AccuracyScorer(Scorer):
    ...
    def is_improving(self, stats):
        return stats.word_accuracy() > self.best_score

    def is_decreasing(self, stats):
        return stats.word_accuracy() < self.best_score

    def _caller(self, stats):
        return stats.word_accuracy()
...
class EarlyStopping(object):
    ...
    def __call__(self, valid_stats, step, chkpt_path):
        """
        ...
        if all([scorer.is_improving(valid_stats) for scorer
            ...
            #old_chkpt = chkpt_path.split("_step_")[0] +"_step_" + self.current_step_best + ".pt"
            if self.best_path is not None:
                subprocess.check_call('rm -f ' + self.best_path, shell=True)
            ...
            self.best_path = chkpt_path
                  ...
                  in self.early_stopping_scorers]):
            self._update_decreasing(chkpt_path)
    ...
    def _update_decreasing(self, chkpt_path):
        ...
        subprocess.check_call('rm -f ' + chkpt_path, shell=True)
    ...
    def _log_best_step(self):
        ...
        logger.info("Best model {}".format(
            self.best_path))
        best_chkpt = self.best_path.split("step_")[0] +"best.pt"
        subprocess.check_call('mv '+ self.best_path  +' '+best_chkpt, shell=True)
